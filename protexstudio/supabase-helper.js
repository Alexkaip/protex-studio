...
function productFromRow(row){return{id:row.id,title:row.name||"",desc:row.description||"",price:row.price||"",category:row.category||"",sizes:splitList(row.sizes||"S,M,L,XL,XXL"),imgFront:row.image||"",imgBack:row.image_back||"",active:row.active!==false};}
function rowFromProduct(product){return{name:product.title,description:product.desc||"",price:String(product.price||""),category:product.category||"",image:product.imgFront||"",image_back:product.imgBack||"",sizes:(product.sizes||[]).join(","),active:product.active!==false};}
