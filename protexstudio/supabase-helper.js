...
function productFromRow(row){return{id:row.id,title:row.name||"",desc:row.description||"",price:row.price||"",category:row.category||"",subcategory:row.subcategory||"",sizes:splitList(row.sizes||"S,M,L,XL,XXL"),imgFront:row.image||"",imgBack:row.image_back||"",imgLeftSleeve:row.image_left_sleeve||"",imgRightSleeve:row.image_right_sleeve||"",active:row.active!==false};}
function rowFromProduct(product){return{name:product.title,description:product.desc||"",price:String(product.price||""),category:product.category||"",subcategory:product.subcategory||"",image:product.imgFront||"",image_back:product.imgBack||"",image_left_sleeve:product.imgLeftSleeve||"",image_right_sleeve:product.imgRightSleeve||"",sizes:(product.sizes||[]).join(","),active:product.active!==false};}
